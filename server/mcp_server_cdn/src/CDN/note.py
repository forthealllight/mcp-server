note = {
   ...
   Returns:
    ...
    Args: 
        body: A JSON structure
             参数 ( 类型 ): 是否必选  描述 
             ---- ( ---- ): ----  ---- 
             Domain ( String ): 是  表示创建哪个域名的配置版本。 
             Message ( String ): 否  表示版本的备注。 
             OriginalVersion ( String ): 否  表示复制哪个版本的配置。 
                   您可以调用 ListDomainVersions 获取域名下的所有版本的版本号。对于指定的版本，您可以调用 DescribeDomainVersion 获取该版本中的配置。 
             ServerSentEvent ( Object of ServerSentEvent ): 否   
             CustomLogField ( Object of CustomLogField ): 否   
            "字段"： ServerSentEvent
             参数 ( 类型 ): 是否必选  描述 
             ---- ( ---- ): ----  ---- 
             Switch ( Boolean ): 否  无 
            "字段"： CustomLogField
             参数 ( 类型 ): 是否必选  描述 
             ---- ( ---- ): ----  ---- 
             CustomLogFieldRule ( Array of CustomLogFieldRule ): 否  CustomLogFieldRule 表示自定义94日志字段配置规则，最多支持50条。 
             Switch ( Boolean ): 否  Switch 表示是否启用该特性。该参数有以下取值： - true：表示启用该特性，需要指定CustomLogFieldRule - false：表示禁用该特性，不需要指定CustomLogFieldRule，若指定则报错。 该参数的默认值是false。 
            "字段"： CustomLogFieldRule
             参数 ( 类型 ): 是否必选  描述 
             ---- ( ---- ): ----  ---- 
             LogFieldKey ( String ): 否  LogFieldKey 表示日志字段名称。 - 支持字母、数字、下划线、连字符、点号 - 长度不超过32个字符 
             LogFieldSource ( String ): 否  LogFieldSource 表示日志字段值采集来源。有如下取值： - request_header：请求头部 - response_header：响应头部 - customize：自定义输入 
             LogFieldValue ( String ): 否  LogFieldValue 表示日志字段值采集。 如果LogFieldSource = request_header、response_header，Value表示从哪个头部取值，输入限制如下： - 支持字母、数字、下划线、连字符、点号 - 长度不超过64个字符 如果LogFieldSource = customize，Value表示自定义输入内容，输入限制如下： - 长度不超过256个字符 
   Returns: 
        参数 ( 类型 ): 描述 
        ---- ( ---- ): ---- 
   ...
   Returns: 
       ...
       "字段"： DomainConfig
        ...
        CustomLogField ( Object of CustomLogField ): 客户自定义日志94字段 
       "字段"： ServerSentEvent
        参数 ( 类型 ): 描述 
        ---- ( ---- ): ---- 
        Switch ( Boolean ): 无 
       "字段"： CustomLogField
        参数 ( 类型 ): 描述 
        ---- ( ---- ): ---- 
        CustomLogFieldRule ( Array of CustomLogFieldRule ): CustomLogFieldRule 表示自定义94日志字段配置规则，最多支持50条。 
        Switch ( Boolean ): Switch 表示是否启用该特性。该参数有以下取值： - true：表示启用该特性，需要指定CustomLogFieldRule - false：表示禁用该特性，不需要指定CustomLogFieldRule，若指定则报错。 该参数的默认值是false。 
       "字段"： CustomLogFieldRule
        参数 ( 类型 ): 描述 
        ---- ( ---- ): ---- 
        LogFieldKey ( String ): LogFieldKey 表示日志字段名称。 - 支持字母、数字、下划线、连字符、点号 - 长度不超过32个字符 
        LogFieldSource ( String ): LogFieldSource 表示日志字段值采集来源。有如下取值： - request_header：请求头部 - response_header：响应头部 - customize：自定义输入 
        LogFieldValue ( String ): LogFieldValue 表示日志字段值采集。 如果LogFieldSource = request_header、response_header，Value表示从哪个头部取值，输入限制如下： - 支持字母、数字、下划线、连字符、点号 - 长度不超过64个字符 如果LogFieldSource = customize，Value表示自定义输入内容，输入限制如下： - 长度不超过256个字符 
    """,
   ...
   Returns: 
    ...
    Args: 
        body: A JSON structure
             参数 ( 类型 ): 是否必选  描述 
             ---- ( ---- ): ----  ---- 
             Domain ( String ): 是  表示一个域名，修改该域名下指定版本中的 CDN 配置。 
             VersionId ( String ): 是  表示一个版本号，修改该版本中的 CDN 特性配置。 
                   您可以调用 ListDomainVersions 获取指定域名下的所有版本号。 
             Message ( String ): 否  表示该版本的新备注。 
             ServerSentEvent ( Object of ServerSentEvent ): 否   
             CustomLogField ( Object of CustomLogField ): 否   
            "字段"： ServerSentEvent
             参数 ( 类型 ): 是否必选  描述 
